...
def generate_presigned_url(
    ...
    endpoint_for_url = settings.SPACES_ENDPOINT
    if "host.docker.internal" in settings.SPACES_ENDPOINT:
        endpoint_for_url = settings.SPACES_ENDPOINT.replace(
...
def ensure_bucket_exists():
    ...
    try:
        ...
        if error_code == "404" or error_code == "NoSuchBucket":
            ...
            try:
                ...
                logger.info(f"Created bucket: {settings.SPACES_BUCKET}")
                print(f"Created bucket: {settings.SPACES_BUCKET}")
                bucket_created = True
            except ClientError as create_error:
                logger.error(
                    f"Failed to create bucket {settings.SPACES_BUCKET}: {create_error}"
                )
                raise
        elif error_code == "403":
            logger.error(f"Access denied to bucket {settings.SPACES_BUCKET}")
            raise HTTPException(
                status_code=500, detail="Storage configuration error: Access denied"
            )
        else:
            logger.error(f"Error checking bucket {settings.SPACES_BUCKET}: {e}")
            raise
    except Exception as e:
        logger.error(
            f"Unexpected error with bucket {settings.SPACES_BUCKET}: {e}", exc_info=True
        )
        raise
    ...
    try:
        ...
        if bucket_created:
            logger.info(f"Set public read policy on bucket: {settings.SPACES_BUCKET}")
            print(f"Set public read policy on bucket: {settings.SPACES_BUCKET}")
        else:
            logger.info(f"Updated public read policy on bucket: {settings.SPACES_BUCKET}")
    except ClientError as e:
