...
async def test_file_upload(client, auth_token):
    ...
    assert presign_response.status_code == 200, f"Presigned URL generation failed: {presign_response.text}"
    ...
    try:
        ...
        assert upload_response.status_code in [200, 201], f"File upload failed with status {upload_response.status_code}: {upload_response.text}"
